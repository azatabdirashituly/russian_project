...
app.post('/getRespFromCultural', async (req, res) => {
    const response = await openai.chat.completions.create({
        ...
        Анализируй ответы определи основные факторы, которые повышают или снижают мотивацию к изучению русского языка. и дай 3 советы для поднятие мотиваций к изучению русского языка. Ограничение 100 слов`}],
    })
...
app.post('/getRespFromCareer', async (req, res) => {
    const response = await openai.chat.completions.create({
        ...
        Анализируй ответы определи основные факторы, которые повышают или снижают мотивацию к изучению русского языка. и дай советы для улучшения, поднятие мотиваций к изучению русского языка В общем 3 совета. Ограничение 100 слов`}],
    })
...
app.post('/getRespFromPersonal', async (req, res) => {
    const response = await openai.chat.completions.create({
        ...
        Анализируй ответы и дай советы для улучшения, поднятие мотиваций к изучению русского языка В общем 3 совета. Ограничение 100 слов
        `}],
    })
